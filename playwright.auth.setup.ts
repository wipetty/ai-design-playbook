...
async function waitForAuthentication(page: Page): Promise<boolean> {
  ...
  try {
    ...
    await page.waitForFunction(
      () => {
        return (
          typeof (window as any).adobeIMSAuthToken === 'string' &&
          (window as any).adobeIMSAuthToken.length > 0
        );
      },
...
async function getAuthInfo(page: Page): Promise<{ token: string; email?: string }> {
  return await page.evaluate(() => {
    const token = (window as any).adobeIMSAuthToken || '';
...
async function setup() {
  ...
  try {
    ...
    const signInTriggered = await page.evaluate(() => {
      ...
      const adobeIMS = (window as any).adobeIMS;
